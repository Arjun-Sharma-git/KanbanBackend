...
router.post(
  ...
  async (req, res) => {
    try {
      ...
      if(!checks.email(req.body.email) || !checks.password(req.body.password) || !checks.name(req.body.name)){
...
router.put(
  ...
  async (req, res) => {
    try {
      ...
      if(!checks.password(req.body.oldPassword) || !checks.name(req.body.name) || checks.password(req.body.newPassword)){
        return res.send({ success: false, data: "Invalid input" });
      }
